...
function alert2(title, html = '', option = {}, success, error){
  var alertOption = {
    icon: ('icon' in option) ? option.icon : '',

    confirmText: ('confirmText' in option) ? option.confirmText : 'Confirmar',
    confirmButton: ('confirmButton' in option) ? option.confirmButton : true,

    cancelText: ('cancelText' in option) ? option.cancelText : 'Fechar',
    cancelButton: ('cancelButton' in option) ? option.cancelButton : (('cancelText' in option) ? true : false ),

    form: ('form' in option) ? option.form : {}
  }

  var optionAlert = {
    'title': title,
    'html': html,
    'type': alertOption.icon,

    'confirmButtonText': alertOption.confirmText,
    'showConfirmButton': alertOption.confirmButton,

    'cancelButtonText': alertOption.cancelText,
    'showCancelButton': alertOption.cancelButton
  };

  swal({...optionAlert, ...alertOption.form}).then((result) => {
    if (result.value) {
      if (typeof success === "function") {
        if( 'input' in alertOption.form){
